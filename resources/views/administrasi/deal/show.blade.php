...
<div class="card deal-task-card mt-3">
    <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2 deal-task-header">
        ...
        <form id="taskFilterForm" method="GET" action="{{ route('administrasi.deal.show', $deal->id) }}" class="d-flex align-items-center justify-content-end gap-2 flex-wrap ms-auto" aria-label="Filter tugas proyek">
            ...
            <button type="button" class="deal-add-task-btn" onclick="openModalCustom('#addTaskModal')">
                + Tambah Tugas
            </button>
            @endif
        </form>
        ...
        <table class="table mb-0 align-middle deal-task-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @php
                            $isReadOnly = ($task->status === 'Done' || $deal->progress >= 100);
                        @endphp
                        @if(!$isReadOnly)
    ...
    <div class="modal fade" id="updateTaskModal{{ $task->id }}" tabindex="-1" aria-labelledby="updateTaskModalLabel{{ $task->id }}" aria-hidden="true">
        <div class="modal-dialog modal-dialog-scrollable">
            <div class="modal-content deal-modal-content">
                <form method="POST" action="{{ route('administrasi.deal.tasks.update', [$deal->id, $task->id]) }}" class="js-track-changes-form">
                    @csrf
                    @method('PATCH')
                    <input type="hidden" name="task_form" value="update">
                    <input type="hidden" name="task_id" value="{{ $task->id }}">
                    <div class="modal-header deal-modal-header">
                        <h5 class="modal-title" id="updateTaskModalLabel{{ $task->id }}">{{ $isReadOnly ? 'Detail Tugas Proyek' : 'Update Tugas Proyek' }}</h5>
                    </div>

                    <div class="modal-body">
                        <div class="mb-3">
                            <label for="nama_tugas_{{ $task->id }}" class="form-label">Nama Tugas</label>
                            <input type="text" id="nama_tugas_{{ $task->id }}" name="nama_tugas" class="form-control" value="{{ $isTaskOldInput ? old('nama_tugas') : $task->nama_tugas }}" required @if($task->nama_tugas === 'Invoice Penagihan' || $isReadOnly) readonly disabled @endif>
                        </div>
                            ...
                            <div class="deal-add-task-members-box">
                                ...
                                <div class="form-check">
                                    <input
                                        ...
                                        type="checkbox"
                                        id="edit_user_{{ $task->id }}_{{ $loop->index }}"
                                        name="anggota[]"
                                        value="{{ $anggotaName }}"
                                        @if(in_array($anggotaName, $effectiveSelectedAnggota)) checked @endif
                                        @if($isReadOnly) disabled @endif>
                                    <label class="form-check-label" for="edit_user_{{ $task->id }}_{{ $loop->index }}">
                                        {{ $anggotaName }}
                                    </label>
                                </div>
                                @endforeach
                                ...
                                <input
                                    ...
                                    value="{{ $isTaskOldInput ? old('anggota_lainnya') : implode(', ', $otherAnggota) }}"
                                    placeholder="Others"
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" id="tanggal_tugas_{{ $task->id }}" name="tanggal_tugas" class="form-control" value="{{ $isTaskOldInput ? old('tanggal_tugas') : \Carbon\Carbon::parse($task->tanggal_tugas)->format('Y-m-d') }}" required @if($isReadOnly) readonly disabled @endif>
                        </div>

                        <div class="mb-3">
                            <label for="deskripsi_{{ $task->id }}" class="form-label">Deskripsi</label>
                            <textarea id="deskripsi_{{ $task->id }}" name="deskripsi" class="form-control" rows="3" placeholder="Deskripsi tugas (opsional)" @if($isReadOnly) readonly disabled @endif>{{ $isTaskOldInput ? old('deskripsi') : $task->deskripsi }}</textarea>
                        </div>
                        ...
                        <div class="mb-0">
                            ...
                            @if($isReadOnly)
                                ...
                                @endif
                            @else
                                <div class="status-selector-group d-flex gap-2">
                                    <input type="radio" class="btn-check" name="status" id="status_on_progress_{{ $task->id }}" value="On Progress" {{ ($isTaskOldInput ? old('status', 'On Progress') : ($task->status ?? 'On Progress')) === 'On Progress' ? 'checked' : '' }}>
                                    <label class="btn btn-outline-warning d-inline-flex align-items-center gap-2 px-3 py-2 rounded-3 border" for="status_on_progress_{{ $task->id }}" style="font-size: 0.86rem; font-weight: 600; cursor: pointer; transition: all 0.2s ease;">
                                        <span class="status-dot" style="width: 8px; height: 8px; border-radius: 50%; background: currentColor; display: inline-block;"></span>
                                        On Progress
                                    </label>

                                    <input type="radio" class="btn-check" name="status" id="status_done_{{ $task->id }}" value="Done" {{ ($isTaskOldInput ? old('status') : ($task->status ?? '')) === 'Done' ? 'checked' : '' }}>
                                    <label class="btn btn-outline-success d-inline-flex align-items-center gap-2 px-3 py-2 rounded-3 border" for="status_done_{{ $task->id }}" style="font-size: 0.86rem; font-weight: 600; cursor: pointer; transition: all 0.2s ease;">
                                        <span class="status-dot" style="width: 8px; height: 8px; border-radius: 50%; background: currentColor; display: inline-block;"></span>
                                        Done
                                    </label>
                                </div>
...
<div class="modal fade deal-add-task-modal" id="addTaskModal" tabindex="-1" aria-labelledby="addTaskModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable deal-add-task-dialog">
        <div class="modal-content deal-modal-content deal-add-task-content">
            <form method="POST" action="{{ route('administrasi.deal.tasks.store', $deal->id) }}">
                @csrf
                <input type="hidden" name="task_form" value="create">
                <div class="modal-header deal-modal-header deal-add-task-header">
                    <h5 class="modal-title deal-add-task-title" id="addTaskModalLabel">Tambah Tugas Proyek</h5>
                </div>

                <div class="modal-body deal-add-task-body">
                    <div class="mb-3">
                        <label for="nama_tugas" class="form-label deal-add-task-label">Nama Tugas</label>
                        <input type="text" id="nama_tugas" name="nama_tugas" class="form-control deal-add-task-input" value="{{ old('nama_tugas') }}" required>
                    </div>

                    <div class="mb-3">
                        <label class="form-label deal-add-task-label">Nama Anggota</label>
                        <div class="deal-add-task-members-box">
                            @forelse($anggotaOptions as $anggotaName)
                            <div class="form-check mb-2">
                                <input
                                    ...
                                    name="anggota[]"
                                    value="{{ $anggotaName }}"
                                    @if(in_array($anggotaName, (array) old('anggota', []))) checked @endif>
                                <label class="form-check-label" for="user_{{ $loop->index }}">
                                    {{ $anggotaName }}
                                </label>
                            </div>
                            @empty
                            <p class="text-muted mb-2">Tidak ada anggota tersedia</p>
                            @endforelse
                            ...
                            <input
                                ...
                                name="anggota_lainnya"
                                class="form-control form-control-sm mt-2 deal-add-task-input"
                                value="{{ old('anggota_lainnya') }}"
                                placeholder="Others">
                        </div>
                    </div>

                    <div class="mb-3">
                        <label for="tanggal_tugas" class="form-label deal-add-task-label">Deadline Tugas</label>
                        <input type="date" id="tanggal_tugas" name="tanggal_tugas" class="form-control deal-add-task-input" value="{{ old('tanggal_tugas') }}" required>
                    </div>

                    <!-- Durasi removed from add-task modal: duration will be calculated from deadline -->

                    <div class="mb-0">
                        <label for="deskripsi" class="form-label deal-add-task-label">Deskripsi</label>
                        <textarea id="deskripsi" name="deskripsi" class="form-control deal-add-task-input deal-add-task-textarea" rows="3" placeholder="Deskripsi tugas (opsional)">{{ old('deskripsi') }}</textarea>
                    </div>
                </div>

                <div class="modal-footer deal-add-task-footer">
                    <button type="button" class="btn btn-sm deal-cancel-btn" data-bs-dismiss="modal">Batal</button>
                    <button type="submit" class="btn btn-sm app-chip deal-invoice-bottom-btn">Simpan</button>
                </div>
            </form>
        </div>
    </div>
...
<div
    id="dealTaskConfig"
    data-open-task-modal="{{ $errors->any() ? '1' : '0' }}"
    data-task-form="{{ old('task_form') }}"
    data-task-id="{{ old('task_id') }}">
</div>

<div class="d-flex align-items-center gap-2 mt-3">
    <a href="{{ route('administrasi.deal.index') }}" class="btn btn-sm app-chip deal-cancel-btn">Keluar</a>
    <a href="{{ route('administrasi.deal.invoice', $deal->id) }}" target="_blank" rel="noopener" class="btn btn-sm app-chip deal-invoice-bottom-btn">Laporan</a>
</div>
...
<style>
    ...
    /* Modern Status Selector styling */
    .status-selector-group .btn-outline-warning,
    .status-selector-group .btn-outline-success {
    ...
    .status-selector-group .btn-outline-warning .status-dot,
    .status-selector-group .btn-outline-success .status-dot {
        background: #94a3b8 !important;
        transition: background-color 0.2s ease !important;
    }

    /* Hover effects for unchecked buttons */
    .status-selector-group .btn-outline-warning:hover {
        background: #fff8e8 !important;
        border-color: #ffe082 !important;
        color: #b7791f !important;
    }
    .status-selector-group .btn-outline-warning:hover .status-dot {
        background: #f0ad17 !important;
    }

    .status-selector-group .btn-outline-success:hover {
        background: #e5f7ea !important;
        border-color: #b9e5c3 !important;
        color: #2c7a42 !important;
    }
    .status-selector-group .btn-outline-success:hover .status-dot {
        background: #2c7a42 !important;
    }

    /* Checked / Selected states (soft backgrounds) */
    .status-selector-group .btn-check:checked + .btn-outline-warning {
        background: #fff8e8 !important;
        border-color: #ffe082 !important;
        color: #b7791f !important;
        box-shadow: 0 2px 4px rgba(240, 173, 23, 0.08) !important;
    }
    .status-selector-group .btn-check:checked + .btn-outline-warning .status-dot {
        background: #f0ad17 !important;
    }
    
    .status-selector-group .btn-check:checked + .btn-outline-success {
        background: #e5f7ea !important;
        border-color: #b9e5c3 !important;
        color: #2c7a42 !important;
        box-shadow: 0 2px 4px rgba(44, 122, 66, 0.08) !important;
    }
    .status-selector-group .btn-check:checked + .btn-outline-success .status-dot {
        background: #2c7a42 !important;
    }
